...
description: You can use XPath to list all nodes in an XML tree. This article provides an example for C# and Visual Basic which lists the nodes of a tree, with each node identified by an XPath expression that specifies the location of the node in the tree.
ms.date: 07/20/2015
dev_langs:
  - "csharp"
  - "vb"
...
Sometimes it is helpful to list all nodes in a tree – for example, to learn exactly how a method or property affects the tree. In the example below for C# and Visual Basic, nodes are listed with each node identified by an XPath expression that specifies the location of the node in the tree.

> [!NOTE] It isn't particularly helpful to execute XPath expressions using LINQ to XML. LINQ to XML queries perform better than XPath expressions and are much more powerful. However, as a way to identify nodes in the XML tree, XPath works well.
